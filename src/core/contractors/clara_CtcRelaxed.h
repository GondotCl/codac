...
namespace codac2
{
    ...
    class CtcRelaxed<N, X, std::index_sequence<Is...>> : public Ctc<CtcRelaxed<N, X>, decltype((void)Is, X{})...>
    {
    public:
        /**
         ...
         */
        CtcRelaxed(const int n_dim)
            : Ctc<CtcRelaxed<N, X>, decltype((void)Is, X{})...>(n_dim * (N + 1)), dim(n_dim), n(N)
        {
            assert(n_dim > 0);
        }
